  ...
  if (speed_ > 0) {
    ...
    if (distance_remaining_ <= 0) {
        ...
        if (outgoing_route_->IsAtEnd()) {
            ...
            if (!incoming_route_->IsAtEnd()) {
              // SetCurrentRoute("incoming");
                ...
                if (passengers_handled != 0) {  // check # getting on or off
                    distance_remaining_ = 0;
                    did_move = true;  // We move if we have gotten passengers?
                } else {      // if nobody got on or off, then skip stop
                  std::cout << "skipped stop: " <<
                  current_route->GetDestinationStop()->GetId() << std::endl;
                }
        ...
        if (passengers_handled != 0) {
            distance_remaining_ = 0;
            did_move = true;
        } else {  // if no passengers handled then stop was skipped
          std::cout << "skipped stop: " <<
          current_route->GetDestinationStop()->GetId() << std::endl;
        }
...
void Bus::Update() {  // using common Update format
  ...
  IObservable::NotifyObservers(data);
}

void Bus::Report(std::ostringstream *out) {
  ...
  (*out) << " Type " << type_;
  (*out) << " Speed " << speed_;
  (*out) << " Distance_to_next_stop " << distance_remaining_;
  (*out) << " Total_number_of_passengers " << GetNumPassengers();
  for (std::list<Passenger *>::iterator it = passengers_.begin();
                                        it != passengers_.end(); it++) {
    (*it)->Report(std::cout);
  }
...
int Bus::UnloadPassengers() {
  ...
  passengers_unloaded = unloader_->UnloadPassengers(passengers_, next_stop_);
...
// indended for use with decorator pattern
// void Bus::SetColor() {
//   std::tuple<int,int,int,int> color = std::make_tuple(255,255,255,1);
//   color_ = color;
// }
//
// void Bus::SetCurrentRoute(std::string route) {
//   current_route_ = route;
// }
//
// std::string Bus::GetCurrentRoute() {
