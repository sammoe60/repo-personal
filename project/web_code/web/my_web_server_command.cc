...
void AddListenerCommand::execute(MyWebServerSession* session,
  picojson::value& command, MyWebServerSessionState* state) {
    ...
    BusWebObserver* obs = new BusWebObserver(session);
    // -calls mySim to delete curr observer (ClearObservers) by looping over
    //   busses and clearing observers for each of them.
    // -Creates a new BusWebObserver
    // -calls visualization_simulator AddListener w/ BusWebObserver and ID
    mySim->AddListener(&id, obs);
}
